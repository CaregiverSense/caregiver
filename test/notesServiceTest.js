...
describe("NotesService", function () {
    ...
    describe("#loadNotesForUser()", function () {
        ...
        it("should load notes for the right user", function () {
            var note1 = new notes_2.Note("Note 1", new Date(), userA.userId, userB.userId, true);
            var note2 = new notes_2.Note("Note 2", new Date(), userC.userId, userB.userId, true);
            var note3 = new notes_2.Note("Note 3", new Date(), userB.userId, userA.userId, true);
            var note4 = new notes_2.Note("Note 4", new Date(), userB.userId, userB.userId, true);
            return db_1.default.getConnection(function (c) {
                return (notes_1.default.addNote(c, note1).
                    ...
                    chai_1.expect(notes[0].content).to.equal("Note 1");
                    chai_1.expect(notes[1].content).to.equal("Note 2");
                    chai_1.expect(notes[2].content).to.equal("Note 4");
                }));
            });
        });
    });
});
//# sourceMappingURL=notesServiceTest.js.map